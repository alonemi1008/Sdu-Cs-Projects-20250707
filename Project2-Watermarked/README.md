...
本项目实现了三种数字水印算法的完整解决方案，专为数字版权保护和图像认证设计。项目包含LSB空域算法、DCT频域算法和DWT-DCT-SVD变换域算法，提供从基础到高级的完整数字水印技术栈。

## 数字水印数学基础

### 1. 数字水印基本原理

数字水印技术基于人类视觉系统(HVS)的特性，在不影响原始媒体感知质量的前提下嵌入隐藏信息。设原始图像为I(x,y)，水印信息为W(x,y)，嵌入过程可表示为：

```
I'(x,y) = I(x,y) + α·W(x,y)
```

其中I'(x,y)为含水印图像，α为嵌入强度因子。
...
```
MSE = (1/MN) Σᵢ₌₁ᴹ Σⱼ₌₁ᴺ [I(i,j) - I'(i,j)]²

PSNR = 10·log₁₀(MAX²/MSE)
```

其中MAX为图像的最大像素值(通常为255)，M×N为图像尺寸。

#### 2.2 结构相似性指数(SSIM)

SSIM从亮度、对比度和结构三个维度评估图像相似性：
...
LSB算法基于像素值的最低有效位对图像视觉效果影响最小的特性。对于8位灰度图像，修改最低位只会产生±1的像素值变化。

#### 1.2 数学表示

设像素值为P，水印位为w∈{0,1}，嵌入过程为：
```
P' = (P & 0xFE) | w
```
即：P' = P - (P mod 2) + w
...
对于M×N的图像，LSB算法的理论嵌入容量为：
```
C = M × N × 1 bit = M × N / 8 bytes
```
...
二维DCT变换定义为：
```
F(u,v) = (2/√(MN)) C(u)C(v) Σᵢ₌₀ᴹ⁻¹ Σⱼ₌₀ᴺ⁻¹ f(i,j)cos((2i+1)uπ/2M)cos((2j+1)vπ/2N)
```
...
DCT算法在频域的中频系数中嵌入水印，平衡不可见性和鲁棒性：

```
F'(u,v) = F(u,v) + α·W(u,v)·|F(u,v)|
```

其中α为嵌入强度，W(u,v)为水印信息，|F(u,v)|为DCT系数的绝对值。
...
其中U和V为正交矩阵，Σ为奇异值对角矩阵。

#### 3.3 水印嵌入过程

1. 对原图像进行DWT分解，得到LL子带
2. 对LL子带进行8×8分块DCT变换
3. 对DCT系数矩阵进行SVD分解
4. 修改奇异值嵌入水印：
   ```
