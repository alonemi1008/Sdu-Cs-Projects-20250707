...
SM3哈希算法基于Merkle-Damgård结构，输出256位哈希值。算法主要包含四个阶段：
...
M' = M || 1 || 0^k || l
```
其中k满足(l + 1 + k) ≡ 448 (mod 512)，使得M'长度为512的倍数。
...
初始哈希值IV = (IV₀, IV₁, IV₂, IV₃, IV₄, IV₅, IV₆, IV₇)，迭代过程为：
```
Vᵢ₊₁ = CF(Vᵢ, Bᵢ)  (i = 0, 1, ..., n-1)
```
其中CF为压缩函数。
...
将512位消息分组B扩展为132个32位字W₀, W₁, ..., W₆₇和W'₀, W'₁, ..., W'₆₃：

```
Wⱼ = { B的第j个字,                    0 ≤ j ≤ 15
      { P₁(Wⱼ₋₁₆ ⊕ Wⱼ₋₉ ⊕ (Wⱼ₋₃ <<<< 15)) ⊕ (Wⱼ₋₁₃ <<<< 7) ⊕ Wⱼ₋₆,  16 ≤ j ≤ 67
...
定义两个布尔函数：
```
FFⱼ(X,Y,Z) = { X ⊕ Y ⊕ Z,                    0 ≤ j ≤ 15
              { (X ∧ Y) ∨ (X ∧ Z) ∨ (Y ∧ Z),   16 ≤ j ≤ 63

GGⱼ(X,Y,Z) = { X ⊕ Y ⊕ Z,                    0 ≤ j ≤ 15  
              { (X ∧ Y) ∨ (¬X ∧ Z),           16 ≤ j ≤ 63
```
...
设A, B, C, D, E, F, G, H为8个32位工作变量，初始值为当前哈希值。
...
```
SIMD_FF(X,Y,Z) = X ⊕ Y ⊕ Z  // 向量异或
SIMD_GG(X,Y,Z) = (X ∧ Y) ∨ (¬X ∧ Z)  // 向量位运算
```

#### 3.3 向量化循环移位

```
SIMD_ROL(X, n) = (X << n) | (X >> (32-n))  // 向量循环移位
```
...
实际测试中，SIMD版本相比基础版本可达到1.5-2.8倍的性能提升。

### 4. 算法安全性分析

SM3的安全性基于：
- **抗碰撞性**: 找到M₁ ≠ M₂使得H(M₁) = H(M₂)的复杂度为O(2¹²⁸)
- **抗第二原像攻击**: 给定M₁，找到M₂ ≠ M₁使得H(M₁) = H(M₂)的复杂度为O(2²⁵⁶)
- **抗原像攻击**: 给定h，找到M使得H(M) = h的复杂度为O(2²⁵⁶)
